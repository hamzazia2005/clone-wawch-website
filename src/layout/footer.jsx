      ...
      <div className='max-w-[1440px] w-full px-5 sm:px-12 py-8 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-y-8'>
        ...
        <div className='col-span-2 sm:col-span-2'>
          <div className='flex items-center gap-2'>
            <Image
              src={data?.title?.logo || '/assets/placeholder.png'}
              //priority={true}
              width={32}
              height={32}
              alt='logo'
          ...
          <p className='text-third font-plus font-medium leading-6 mt-4'>
            {data?.copyright?.[0]?.children?.length &&
              data.copyright[0].children.map((child, i) => {
                if (child.type === 'text') {
                ...
                if (child.type === 'link') {
                  return (
                    <Link
                      key={i}
                      href={child.url}
                      className='text-secondary hover:text-green-900 underline underline-offset-4 transition-colors duration-200'
                    >
